...
async def _resolve_booking_url(
    ...
    # CR-005: each attempt acquires its own connection and releases it before
    # sleeping, so polling does not pin a pool slot for the full retry window.
    ...
    for _attempt in range(3):
        ...
        try:
            async with conn.transaction():
                ...
                # Lock not acquired — another instance is mid-creation.
                # Re-query so we can return its token once committed, instead
                # of giving up immediately and leaving the customer without a link.
                ...
                if last_row and last_row["meeting_token"]:
                    logger.info(
                        "Advisory lock missed — reusing token — phone: %s",
                        mask_phone(customer_phone),
                    )
                    return f"{DASHBOARD_URL}/book/{last_row['meeting_token']}"
        finally:
            try:
                await database.pool.release(conn)
            except Exception as e:
                logger.error("Failed to release connection: %s", e)
        # Connection released above before we sleep, so polling does not pin
        # a pool slot for the full retry window.
